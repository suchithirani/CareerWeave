...
const OfficerInterviewSchedule = () => {
  ...
  const fetchSchedules = async () => {
    ...
    setError(null);
    try {
      const response = await api.get('/interview-schedules/officer', {
        headers: {
  ...
  const handleDateTimeChange = (date) => {
    setNewSchedule(prev => ({ ...prev, interviewDateTime: date }));
  };

  const handleStatusChange = (e) => {
    setNewSchedule(prev => ({ ...prev, status: e.target.value }));
  };
  ...
  const handleSubmit = async () => {
    try {
      if (newSchedule.id) {
        await axios.put(`/api/interview-schedules/${newSchedule.id}`, newSchedule, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        ...
        await axios.post('/api/interview-schedules', newSchedule, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleEdit = (schedule) => {
    setNewSchedule({
      ...schedule,
      interviewDateTime: new Date(schedule.interviewDateTime)
    });
    setEditDialog(true);
  };
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/interview-schedules/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
          ...
          <div className="bg-white/60 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl border border-white/50 p-4 sm:p-6 mb-6 sm:mb-8">
            <div className="flex flex-col gap-3 sm:gap-4">
              <div className="relative flex-grow">
                ...
                <input
                  type="text"
                  placeholder="Search by candidate, job title or interviewer..."
                  value={searchTerm}
                        ...
                        {currentItems.map((schedule) => (
                          ...
                          >
                            <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                              <div className="font-medium text-gray-900">{schedule.candidateName}</div>
                            </td>
                ...
                <div className="sm:hidden space-y-3 p-3">
                  {currentItems.map((schedule) => (
                    <div key={schedule.id} className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
                      ...
                      >
                        <div>
                          <h3 className="font-medium text-gray-900">{schedule.candidateName}</h3>
                          <div className="flex items-center mt-1">
                            <span className="text-sm text-gray-600">{schedule.jobTitle}</span>
                          </div>
                        </div>
                      ...
                      <div className="px-4 pb-4 pt-2 border-t border-gray-100">
                        ...
                        <div className="flex justify-end space-x-2">
                          <button
                            ...
                            className="text-blue-600 hover:text-blue-900 text-sm flex items-center px-3 py-1 border border-blue-200 rounded-lg"
                          >
                          ...
                          <button
                            ...
                            className="text-red-600 hover:text-red-900 text-sm flex items-center px-3 py-1 border border-red-200 rounded-lg"
                          >
                ...
                >
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h2 className="text-3xl font-bold text-gray-900 mb-2">{viewSchedule.candidateName}</h2>
                      ...
                      <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-800">
                              {viewSchedule.jobApplicationId || 'Not specified'}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <h2 className="text-2xl font-bold text-gray-900">
                        {newSchedule.id ? 'Edit Interview Schedule' : 'Create New Interview Schedule'}
                      </h2>
                      <button
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <input
                          type="text"
                          name="candidateName"
                          value={newSchedule.candidateName}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={newSchedule.interviewDateTime ? new Date(newSchedule.interviewDateTime).toISOString().slice(0, 16) : ''}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Job Application ID</label>
                        <input
                          type="text"
                          name="jobApplicationId"
                          value={newSchedule.jobApplicationId || ''}
                          onChange={handleInputChange}
                          className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                        />
                      </div>
                      ...
                      <div className="flex justify-end pt-6 border-t border-gray-200 space-x-3">
                        ...
                        <button
                          ...
                          className="px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl text-white hover:from-blue-700 hover:to-indigo-700 transition-colors font-medium"
                        >
                          {newSchedule.id ? 'Update' : 'Create'} Schedule
                        </button>
                      </div>
                    </div>
                  </div>
...
export default OfficerInterviewSchedule;