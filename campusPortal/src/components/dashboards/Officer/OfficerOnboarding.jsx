...
const OfficerOnboarding = () => {
    ...
    const fetchOnboardings = async () => {
        ...
        setError(null);
        try {
            const response = await api.get('/onboarding/officer', {
                headers: {
    ...
    const filteredOnboardings = onboardings.filter(onboarding => {
        const matchesSearch = 
            ...
            (onboarding.company?.toLowerCase().includes(searchTerm.toLowerCase()));
        
        const matchesStatus = 
            filters.status === '' ||
            onboarding.onboardingStatus === filters.status;
        
        const matchesCompany = 
            filters.company === '' ||
            onboarding.company?.toLowerCase().includes(filters.company.toLowerCase());
        
        const matchesFromDate = 
            !filters.fromDate ||
            new Date(onboarding.startDate) >= new Date(filters.fromDate);
        
        const matchesToDate = 
            !filters.toDate ||
            new Date(onboarding.startDate) <= new Date(filters.toDate);
    ...
    const currentItems = filteredOnboardings.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const getStatusBadge = (status) => {
        switch (status?.toUpperCase()) {
            ...
            case 'IN_PROGRESS':
                return <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> In Progress</span>;
            case 'COMPLETED':
                            ...
                            <div className="flex items-center">
                                <div className="relative">
                                    ...
                                    <div className="absolute -top-1 -right-1 w-4 h-4 sm:w-6 sm:h-6 bg-green-500 rounded-full flex items-center justify-center">
                                        <CheckCircle className="w-2 h-2 sm:w-3 sm:h-3 text-white animate-pulse" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <p className="text-gray-600 text-sm sm:text-lg font-medium">
                                        {filteredOnboardings.length} {filteredOnboardings.length === 1 ? 'record' : 'records'} found
                                    </p>
                                </div>
                            </div>
                        ...
                        {loading ? (
                            ...
                            <div className="p-6 sm:p-12 text-center">
                                <div className="w-16 h-16 sm:w-24 sm:h-24 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                                    <UserCheck className="w-6 h-6 sm:w-10 sm:h-10 text-gray-400" />
                                </div>
                                <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-1 sm:mb-2">No onboarding records found</h3>
                                <p className="text-gray-600 max-w-md mx-auto mb-4 sm:mb-6 text-sm sm:text-base">
                                    {filters.status || filters.company || filters.fromDate || filters.toDate || searchTerm
                                        ? "Try adjusting your search criteria"
                                        : "No onboarding records have been created yet"}
                                </p>
                                {(filters.status || filters.company || filters.fromDate || filters.toDate || searchTerm) && (
                                    <button
                                        onClick={resetFilters}
                            ...
                            <>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gradient-to-r from-gray-50 to-blue-50">
                                            <tr>
                                                ...
                                                <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Documents</th>
                                                <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            <AnimatePresence>
                                                {currentItems.map((onboarding) => (
                                                    <motion.tr
                                                        key={onboarding.id}
                                                        layout
                                                        initial={{ opacity: 0 }}
                                                    ...
                                                    >
                                                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                                            <div className="font-medium text-gray-900">{onboarding.candidateName || '-'}</div>
                                                        </td>
                                                        ...
                                                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                            {formatDocuments(onboarding.documentsSubmitted)}
                                                        </td>
                                                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                                            <div className="flex justify-start items-center space-x-2">
                                                                <motion.button
                                                                    ...
                                                                    whileTap={{ scale: 0.9 }}
                                                                    onClick={() => setViewOnboarding(onboarding)}
                                ...
                                <div className="sm:hidden space-y-3 p-3">
                                    {currentItems.map((onboarding) => (
                                        <div key={onboarding.id} className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
                                            <div 
                                                className="p-4 flex justify-between items-center"
                                                onClick={() => setViewOnboarding(onboarding)}
                                            >
                                                <div>
                                                    <h3 className="font-medium text-gray-900">{onboarding.candidateName || '-'}</h3>
                                                    <div className="flex items-center mt-1">
                                                        <span className="text-sm text-gray-600">{onboarding.jobTitle || '-'}</span>
                                                    </div>
                                                </div>
                                ...
                                {/* Pagination */}
                                {filteredOnboardings.length > itemsPerPage && (
                                ...
                                >
                                    <div className="p-8">
                                        <div className="flex justify-between items-start mb-6">
                                            <div>
                                                <h2 className="text-3xl font-bold text-gray-900 mb-2">{viewOnboarding.candidateName || 'Candidate'}</h2>
                                                <div className="flex items-center space-x-4">
                                                    <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                                                        {viewOnboarding.jobTitle || 'Job Title'}
                                                    </span>
                                                    {getStatusBadge(viewOnboarding.onboardingStatus)}
                                                </div>
                                            </div>
                                            <button
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                                            ...
                                            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-6">
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div>
                                                        <h4 className="text-sm font-medium text-gray-500 mb-1">Documents Submitted</h4>
                                                        <p className="text-gray-800">
                                                            {formatDocuments(viewOnboarding.documentsSubmitted)}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <h4 className="text-sm font-medium text-gray-500 mb-1">Remarks</h4>
                                                        <p className="text-gray-800">
                                                            {viewOnboarding.remarks || '-'}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
